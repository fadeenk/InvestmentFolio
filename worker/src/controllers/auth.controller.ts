...
import { fetchAccountNumbers } from '../services/schwab-trader.service'
...
async function updateConnectedAccountCount(workerEnv: WorkerEnv, envelope: TokenEnvelope): Promise<TokenEnvelope> {
	const accounts = await fetchAccountNumbers(workerEnv, envelope.accessToken)
	const connectedAccountCount = accounts.length
	if (connectedAccountCount === envelope.connectedAccountCount) {
		return envelope
	}

	const nextEnvelope: TokenEnvelope = {
		...envelope,
		connectedAccountCount,
	}

	const encryptedNext = await encryptTokenEnvelope(workerEnv, nextEnvelope)
	await putEncryptedTokens(workerEnv, encryptedNext)
	return nextEnvelope
}
...
export async function handleAuthRefresh(env: Env): Promise<Response> {
	...
	try {
		const next = await refreshEnvelope(workerEnv, current)
		const payload: RefreshResponse = {
			success: true,
...
export async function handleAuthStatus(env: Env): Promise<Response> {
	...
	try {
		let envelope = await decryptTokenEnvelope(workerEnv, encrypted)
		const nowMs = Date.now()
		...
		if (!isAccessTokenValidAt(envelope, nowMs)) {
			try {
				envelope = await refreshEnvelope(workerEnv, envelope)
			} catch {
		...
		try {
			envelope = await updateConnectedAccountCount(workerEnv, envelope)
		} catch {
			// Keep status endpoint healthy if live count fetch fails.
		}
